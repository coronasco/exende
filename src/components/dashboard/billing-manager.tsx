  ...
  return (
    <>
      <header className="dashboard-heading">
        <div><p className="eyebrow">Subscription & entitlement</p><h1>Billing</h1><p>Stripe handles checkout, payment methods, invoices, and subscription changes. Exende grants credits only after confirmed payment.</p></div>
      ...
      {!billing.configured ? <p className="billing-notice" data-tone="warning">Stripe test-mode configuration is not connected to this deployment yet. Plan selection remains disabled.</p> : null}
      ...
      <div className="billing-policy"><ShieldCheck /><div><strong>Credits follow paid periods.</strong><p>Monthly grants expire at the end of their billing period and do not roll over. Failed payments do not create new grants; existing unexpired credits remain available during the past-due state.</p></div></div>

      <section className="billing-plan-grid" aria-label="Available plans">
        {billing.plans.map((plan) => {
          ...
          return <article key={plan.key} className="billing-plan" data-featured={plan.key === "pro"} data-current={current}>
            <div><span className="annotation">{plan.key === "pro" ? "PRODUCTION" : plan.key.toUpperCase()}</span>{current ? <span className="billing-plan__current"><Check /> Current</span> : null}</div>
            <h2>{plan.name}</h2><strong>{price(plan.priceEurMonthly)}<small> / month</small></strong><p>{plan.description}</p>
            <ul><li><Check /> {formatNumber(plan.monthlyCredits)} monthly credits</li><li><Check /> Scoped customer API keys</li><li><Check /> Usage and request metadata</li><li><Check /> Stripe billing portal</li></ul>
            <button type="button" disabled={current || !billing.configured || pending !== null} onClick={() => billing.customerPortalAvailable ? openPortal() : openCheckout(plan.key)}>{current ? "Current plan" : pending === plan.key || pending === "portal" ? "Opening Stripe..." : billing.customerPortalAvailable ? `Change to ${plan.name}` : `Select ${plan.name}`} {!current ? <ArrowRight /> : null}</button>
          </article>;
        })}
      </section>
    </>
